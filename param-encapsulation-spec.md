...
As built in Phase 3: fifteen scenarios, ninety-two checks a round, and the list above is now written in full. Narrowing sending no requests reads vertico's own count rather than the completion table, because the table serves everything fetched and the filter sits between it and the display. Abort mid-stream needed a delay knob in the stub, since at the usual delivery delay the chain finishes before a poll can interrupt it. The legacy string is pinned end to end as well as by its unit specs, including that it outranks the state object.
...
Phase 3, input semantics and documentation. The section 9 decision, which came back as keep what is already there, the obsoletion of `consult-hn-initial-input-string`, `make test`, readme and changelog, e2e for narrowing sending no requests. Done, and it also closed the two scenarios section 13 had listed and never grown, put the compile gate into CI, and left the branch releasable rather than merely finished.
